...
void *depth_thread(){
	...
	while(1){
		...
		if(refcount){
			...
			if(refcount == -1) refcount=0; else refcount--;
			if(refcount){
				pthread_mutex_lock(&depth_mutex);	
				dcb++;
				dwait--;
		...
		do{
			...
			do{
				...
				if(gmin != gmax){
					...
					for(;gmin != gmax;gmin++){ depth_to_gray[gmin] = ((ni-depth_to_mm[gmin])/ftmp)*255;}
					#if defined USE_SSE
					_mm_empty();
					#endif
				}
...
void *video_thread(){
	...
	while(1){
		...
		do{
			...
			pthread_mutex_unlock(&net_mutex);
			if(!vframe){ CFRAME(vframe,vhead,65536); }
...
int main(int argc, char **argv){	
	...
	while (!die){
		while(freenect_process_events(f_ctx) < 0){
			...
			do{ itmp = serial ? freenect_open_device_by_camera_serial(f_ctx, &f_dev, serial):freenect_open_device(f_ctx, &f_dev, camera); }while(itmp < 0);
			freenect_set_depth_buffer(f_dev,depth_raw[0]);
			freenect_set_depth_mode(f_dev, freenect_find_depth_mode(FREENECT_RESOLUTION_MEDIUM, FREENECT_DEPTH_11BIT_PACKED));
			freenect_set_depth_callback(f_dev, depth_cb);
			freenect_set_video_buffer(f_dev,video_raw[0]);
							 ...
							 while(oarg){
									if(!strncmp(targ,"Origin:",7)){
										...
										if(oarg){
											...
											if(!admin) LIST_FOREACH(origin, &origins, entries) if(!strncmp(origin->n,oarg,origin->l)){ kick=0;break;}
											break;
										}
									}
									targ=strtok(NULL," \r\n");
