 ...
 * for more details.
 * 
 * Slightly modded by wizgrav@gmail.com to meet the needs of intrael
 */
...
struct _SkeltrackSkeletonPrivate
{
  ...
  guint16 distance_threshold_square;
  ...
  guint32 shoulders_offset_square;
...
skeltrack_skeleton_init (SkeltrackSkeleton *self)
{
  ...
  priv->distance_threshold_square = priv->distance_threshold*priv->distance_threshold;
  ...
  priv->shoulders_offset_square = SHOULDERS_OFFSET*SHOULDERS_OFFSET;
...
{
  Node *neighbor;
  if (i < self->priv->bound_x || i >= self->priv->bound_X ||
      j < self->priv->bound_y || j >= self->priv->bound_Y)
    {
      return index;
    }
...
make_graph (SkeltrackSkeleton *self, GList **label_list)
{
  ...
  bx = self->priv->bound_x;
  bX = self->priv->bound_X;
  by = self->priv->bound_y;
  bY = self->priv->bound_Y;
  ...
  for (current_extrema = g_list_first (extremas);
       ...
       current_extrema = g_list_next (current_extrema))
    {
      ...
      if (right_shoulder_closest_point->i < node->i &&
          ...
          shoulders_distance <= shoulders_maximum_distance_sqrt)
        {
          *head = node;
...
static SkeltrackJoint **
track_joints_centroid (SkeltrackSkeleton *self, guint ax, guint ay, guint az)
{
  Node * centroid;
  Node *head = NULL;
  Node *cent = NULL;
...
SkeltrackJointList
 skeltrack_skeleton_track_joints_intrael (SkeltrackSkeleton   *self,
                                 guint16             *buffer,
                                 guint                width,
                                 guint                height,
                                 guint				  hasCentroid,
                                 ...
                                 guint				   bY  )
{
  ...
  self->priv->buffer_height = height;
  self->priv->bound_x=bx;
  self->priv->bound_y=by;
  self->priv->bound_X=bX;
  self->priv->bound_Y=bY;
  
  SkeltrackJointList joints = hasCentroid ? track_joints_centroid (self,ax,ay,az):track_joints(self);
  return (SkeltrackJointList) joints;
}
