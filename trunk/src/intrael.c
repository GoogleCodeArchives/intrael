...
#define BIND(NAME,ADDR,VAR) {\
 	...
 	if(bind(NAME, (struct sockaddr *)&ADDR, sizeof(ADDR)) == SOCKET_ERROR) FAIL("error on bind() for #NAME")\
 	if(listen(NAME, 10) == SOCKET_ERROR) FAIL("error on listen() for #NAME")\
 	FD_SET(NAME,&master);\
	}
#define CSWITCH(ch,optarg) switch (ch) {\
			    ...
			    case 'v': printf("\nIntrael v0. (C) 2011 Yannis Gravezas. Check http://www.intrael.com for more info.\n\n");\
				      return 0;\
			    ...
			    case 'S': if(strlen(optarg))\
				      ...
				      break;\
			    case 'b': lf = strncpy(loadfile,optarg,1024);\
				      break;\
			    case 'o': sf = strncpy(savefile,optarg,1024);\
				      break;\
			    }
#define RSWITCH(ch,optarg) switch (ch) {\
			    ...
			    case 'w': IOPT(optarg,t_w,0,6400000);\
			    case 'W': IOPT(optarg,t_W,0,6400000);\
			    case 'h': IOPT(optarg,t_h,0,4800000);\
			    case 'H': IOPT(optarg,t_H,0,4800000);\
			    ...
			    case 'A': IOPT(optarg,t_a,0,1);\
			    case 'i': IOPT(optarg,instance,0,2147483647);\
			    case 'B': if(strlen(loadfile)) lf=loadfile;\
				      break;\
			    case 'O': if(strlen(savefile)) sf=savefile;\
				      break;\
			    }
...
char message[4096],httpmessage[4096],nmessage[4096],nhttpmessage[4096],tempmessage[256],confmessage[4096], loadfile[1024], savefile[1024],secret[32],*lf,*sf;
uint16_t depth_ref[FRAME_PIXELS],depth_ref_z[FRAME_PIXELS],depth_ref_Z[FRAME_PIXELS],depth_ref_x[FRAME_PIXELS],depth_ref_y[FRAME_PIXELS],depth_to_raw[16384],depth_to_mm[2048],written,headwritten,tempwritten,nwritten,nheadwritten;
uint32_t run_s[FRAME_PIXELS/2+1],run_e[FRAME_PIXELS/2+1],run_z[FRAME_PIXELS/2+1],run_Z[FRAME_PIXELS/2+1],run_sum[FRAME_PIXELS/2+1],run_label[FRAME_PIXELS/2+1],r_label[FRAME_PIXELS/2+1],l_pos_x[FRAME_PIXELS/2+1],l_pos_X[FRAME_PIXELS/2+1],l_pos_y[FRAME_PIXELS/2+1],l_pos_Y[FRAME_PIXELS/2+1],l_pos_z[FRAME_PIXELS/2+1],l_pos_Z[FRAME_PIXELS/2+1],l_cx[FRAME_PIXELS/2+1],l_cy[FRAME_PIXELS/2+1],l_sum[FRAME_PIXELS/2+1],l_count[FRAME_PIXELS/2+1],l_runs[FRAME_PIXELS/2+1],l_reg[FRAME_PIXELS/2+1],l_checked[FRAME_PIXELS/2+1],t_x,t_X,t_y,t_Y,t_z,t_Z,t_f,t_F,t_d,t_D,t_w,t_W,t_h,t_H,t_c,t_C,t_c,connections, counter,ncounter, gstamp,instance,lport,mport;
int mode,cmode,refcount,angle,t_a,reset,reled,yes,itmp;
	...
	if(refcount>0){	 	
		...
		if(refcount > 0){
		 for(i = 0; i < FRAME_PIXELS; i++)
		 	...
		 	if(lf){
				if((fp = fopen(lf,"rb"))){
					if(fread(depth, FRAME_PIXELS*2, 1, fp) != FRAME_PIXELS*2 ){ 
						 ERR("could not load a reference frame from the specified file",' ') 
					}else
					 	memcpy(depth_ref,depth,FRAME_PIXELS*2);
					fclose(fp);
				}else 
				 	ERR("could not load the reference frame from the specified file",' ')
				lf = NULL;
			}
			for(i = 0;i < FRAME_PIXELS;i++){
			 	...
			 	if(depth_ref_x[i]>t_X) 
					...
					if(dp > t_z) depth_ref_z[i] = depth_to_raw[t_z];
					else if(dp > t_z-mode) depth_ref_z[i] = depth_to_raw[t_z-mode];
					if(dp > t_Z) depth_ref_Z[i] = depth_to_raw[t_Z];
					else if(dp > t_Z-mode) depth_ref_Z[i] = depth_to_raw[t_Z-mode]; 
				}else{
			...
			if(sf){
				if((fp = fopen(sf,"wb"))){
					fwrite(depth_ref, FRAME_PIXELS*2, 1, fp);
				 	...
				 	ERR("could not save the reference frame to the specified file",' ');
				sf = NULL;
			}
		}
	}else{
		...
		counter++;
		written=sprintf(message,"%u,%u,%u,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%f,%f,%d,%d,%f,%f,%d,%d,%f,%f,%f,%d,%d,%u",counter,timestamp,instance,mode,t_x,t_X,t_y,t_Y,t_z,t_Z,t_w,t_W,t_h,t_H,t_f,t_F,t_d,t_D,t_t,t_T,t_c,t_C,t_q,t_Q,angle,t_a,state->accelerometer_x/819*9.80665, state->accelerometer_y/819*9.80665, state->accelerometer_z/819*9.80665,((int)(((double)state->tilt_angle) / 2.)), state->tilt_status,dp);
		...
		for(j=0;j!=dp;j++){
			...
			if(written+tempwritten>3948) break;
...
{	
	...
	while (!die && freenect_process_events(f_ctx) >= 0) { 
		...
		if(t_a) freenect_update_tilt_state(f_dev);
...
int main(int argc, char **argv)
{
 ...
 depth_to_mm[2047] = manager = listener = t_x = t_y = t_f = t_F = t_d = t_D = t_w = t_W = t_h = t_H = t_C = t_q = t_Q = t_t = t_T = mport = mode = cmode =  t_a = gstamp = camera = instance = 0;
 lf = sf  = NULL;
