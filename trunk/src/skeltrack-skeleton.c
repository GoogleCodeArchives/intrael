...
struct _SkeltrackSkeletonPrivate
{
  ...
  guint32 distance_threshold_square;
  guint32 hands_minimum_distance_square;
  guint32 shoulders_minimum_distance_square;
  guint32 shoulders_maximum_distance_square;
  
  //WIZGRAV Bounding box
  guint16 bound_left;
  guint16 bound_top;
  guint16 bound_right;
  guint16 bound_bottom;
...
skeltrack_skeleton_class_init (SkeltrackSkeletonClass *class)
{
  ...
  g_object_class_install_property (obj_class,
                         PROP_BOUND_LEFT,
                         g_param_spec_uint ("bound-left",
                                            "Leftmost bound",
                                            "The minimum x coordinate for the "
                                            "bounding box.",
                                            0,
                                            G_MAXUINT16,
                                            0,
 ...
 g_object_class_install_property (obj_class,
                         ...
                         g_param_spec_uint ("bound-top",
                                            "Topmost bound",
                                            "The minimum y coordinate for the "
                                            "bounding box.",
                                            0,
                                            G_MAXUINT16,
                                            0,
 ...
 g_object_class_install_property (obj_class,
                         ...
                         g_param_spec_uint ("bound-right",
                                            "Rightmost bound",
                                            "The maximum x coordinate for the "
                                            "bounding box.",
                                            1,
                                            G_MAXUINT16,
                                            G_MAXUINT16,
 ...
 g_object_class_install_property (obj_class,
                         ...
                         g_param_spec_uint ("bound-bottom",
                                            "Bottommost bound",
                                            "The maximum y coordinate for the "
                                            "bounding box.",
                                            1,
                                            G_MAXUINT16,
                                            G_MAXUINT16,
...
skeltrack_skeleton_init (SkeltrackSkeleton *self)
{
  ...
  priv->distance_threshold_square = GRAPH_DISTANCE_THRESHOLD*GRAPH_DISTANCE_THRESHOLD;
  ...
  priv->hands_minimum_distance_square = HANDS_MINIMUM_DISTANCE*HANDS_MINIMUM_DISTANCE;
  priv->shoulders_minimum_distance_square = SHOULDERS_MINIMUM_DISTANCE*SHOULDERS_MINIMUM_DISTANCE;
  priv->shoulders_maximum_distance_square = SHOULDERS_MAXIMUM_DISTANCE*SHOULDERS_MAXIMUM_DISTANCE;
                                 ...
                                 GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_GRAPH_DISTANCE_THRESHOLD:
      ...
      self->priv->distance_threshold_square = self->priv->distance_threshold*self->priv->distance_threshold;
      break;
    ...
    case PROP_HANDS_MINIMUM_DISTANCE:
      ...
      self->priv->hands_minimum_distance_square = self->priv->hands_minimum_distance*self->priv->hands_minimum_distance;
      break;

    case PROP_SHOULDERS_MINIMUM_DISTANCE:
      self->priv->shoulders_minimum_distance = g_value_get_uint (value);
      self->priv->shoulders_minimum_distance_square = self->priv->shoulders_minimum_distance * self->priv->shoulders_minimum_distance ;
      break;

    case PROP_SHOULDERS_MAXIMUM_DISTANCE:
      self->priv->shoulders_maximum_distance = g_value_get_uint (value);
      self->priv->shoulders_maximum_distance_square = self->priv->shoulders_maximum_distance*self->priv->shoulders_maximum_distance;
      break;
	...
	case PROP_BOUND_LEFT:
      self->priv->bound_left = g_value_get_uint (value);
      break;
    
    case PROP_BOUND_RIGHT:
      self->priv->bound_right = g_value_get_uint (value);
      break;
    
    case PROP_BOUND_TOP:
      self->priv->bound_top = g_value_get_uint (value);
      break;
    
    case PROP_BOUND_BOTTOM:
      self->priv->bound_bottom = g_value_get_uint (value);
      break;
                                 ...
                                 GParamSpec *pspec)
{
  ...
  switch (prop_id)
	...
	case PROP_BOUND_LEFT:
      g_value_set_uint (value, self->priv->bound_left);
      break;

	case PROP_BOUND_TOP:
      g_value_set_uint (value, self->priv->bound_top);
      break;

	case PROP_BOUND_RIGHT:
      g_value_set_uint (value, self->priv->bound_right);
      break;

	case PROP_BOUND_BOTTOM:
      g_value_set_uint (value, self->priv->bound_bottom);
      break;
...
static gint
get_distance_square (Node *a, Node *b)
{
  guint dx, dy, dz;
  dx = ABS (a->x - b->x);
  dy = ABS (a->y - b->y);
...
get_closest_node (GList *node_list, Node *from)
{
  ...
  for (current_node = g_list_first (node_list);
    ...
    {
      Node *node;
      gint32 current_distance;
      node = (Node *) current_node->data;
      if (closest == NULL)
        {
          closest = node;
          distance = get_distance_square (node, from);
          continue;
        }
      current_distance = get_distance_square (node, from);
...
{
  Node *neighbor;
  if (i < self->priv->bound_left || i >= self->priv->bound_right ||
      j < self->priv->bound_top || j >= self->priv->bound_bottom)
    {
      return index;
    }
  ...
  if (neighbor != NULL)
    {
      gint32 distance;
      distance = get_distance_square (neighbor, node);
      if (distance < self->priv->distance_threshold_square)
        {
  ...
  for (current_node = g_list_first (nodes);
       ...
       current_node = g_list_next (current_node))
    {
      ...
      if (node->label->bridge_node == NULL)
        ...
        {
          Node *closest_node;
          gint32 bridge_distance, distance;
          bridge_distance = get_distance_square (node->label->bridge_node,
                                          node->label->to_node);
          closest_node = get_closest_node (lowest_component, node);
          distance = get_distance_square (node,
                                   closest_node);
...
make_graph (SkeltrackSkeleton *self, GList **label_list)
{
  ...
  guint16 value;
  guint16 *buffer;
  gint width, height,bx,bX,by,bY;

  buffer = self->priv->buffer;
  width = self->priv->buffer_width;
  height = self->priv->buffer_height;
  
  //WIZGRAV: Check bounds
  bx = MAX(self->priv->bound_left,0);
  bX = MIN(self->priv->bound_right,width);
  if(bx >= bX){
	bx=0;
	bX=width; 
  }
  by = MAX(self->priv->bound_top,0);
  bY = MIN(self->priv->bound_bottom,height);
  if(by >= bY){
	by=0;
	bY=height;  
  }
...
assign_extremas_to_left_and_right (Node *extrema_a,
                                   Node *extrema_b,
                                   gint32 distance_left_a,
                                   gint32 distance_left_b,
                                   gint32 distance_right_a,
                                   gint32 distance_right_b,
                                   Node **left_extrema,
                                   Node **right_extrema)
{
  gint index;
...
{
  gint32 *dist_left_a = NULL;
  gint32 *dist_left_b = NULL;
  gint32 *dist_right_a = NULL;
  gint32 *dist_right_b = NULL;
  gint32 total_dist_left_a = -1;
  gint32 total_dist_right_a = -1;
  gint32 total_dist_left_b = -1;
  gint32 total_dist_right_b = -1;
  gint32 *distances_right, *distances_left;
